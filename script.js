...
input.onblur = function() {
  input.placeholder = "Wpisz tekst, aby zacząć...";
};
...
function setColors() {
  if (green === 255 && red < 255 && blue === 0) {
    ...
    for (var i = 1; i < 4; i++) {
      document.getElementsByClassName("movers")[i].style.transform =
...
input.oninput = function() {
  ...
  if (input.value === words[level][pointer]) {
    setColors();
    setScore();
    if (level === 0 && pointer >= 19 && pointer != 29) countdown = 3;
    else countdown = 4;
    document.getElementById("secs").innerHTML = countdown;
    if (blue === 255) {
      var n = level + 1;
